...
function addCandle(forced=null){
  ...
  function normalCandle(){
    ...
    if(run.regime === "range"){
      ...
      if(distHigh < 2.2) bias -= 0.62;
      if(distLow < 2.2) bias += 0.62;
    } else {
    ...
    const noise = (Math.random() - 0.5) * run.volatility * 1.18;
    const body = bias + noise;
    c = clampToWorld(o + body);

    const wickBase = 0.28 + Math.random() * run.volatility * 0.72;
  ...
  if(!forced){
    ...
    if(p==="Bullish Engulfing"){
      ...
      o=base-0.7;c=base+1.75;h=c+0.38;l=o-0.38;
    }
    else if(p==="Bearish Engulfing"){
      const base = Math.min(R - 1.2, prev + 0.55);
      // Shape the current candle only; avoids double-print gaps.
      o=base+0.7;c=base-1.75;h=o+0.38;l=c-0.38;
    }
    else if(p==="Hammer"){
...
function drawLevelLabel(ctx,text,x,y,color){
  ...
  const h = 20;
  ctx.fillStyle="rgba(12,20,15,.78)";
...
function drawGame(frozen=false){
  ...
  ctx.setLineDash([8,6]);ctx.lineWidth=2.6;
  ...
  ctx.setLineDash([3,6]);ctx.strokeStyle="rgba(142,92,247,.75)";ctx.beginPath();ctx.moveTo(30,mapY(mid));ctx.lineTo(W-24,mapY(mid));ctx.stroke();ctx.setLineDash([]);drawLevelLabel(ctx,"Range High",36,mapY(hi)-18,"#ff9f1c");
  drawLevelLabel(ctx,"Midpoint",36,mapY(mid)-18,"#8e5cf7");
  drawLevelLabel(ctx,"Range Low",36,mapY(lo)+8,"#1cb0f6");
